...
a.updated {
	...
	dat += "<span class='user'>Welcome, <b>[station_name()] Cargo Department</b></span><br><span class='balance'><b>Credits:</b> [D.account_balance] </span><br>"
...
/obj/machinery/computer/stockexchange/proc/buy_some_shares(var/datum/stock/S, var/mob/user)
	...
	var/datum/bank_account/D = SSeconomy.get_dep_account(ACCOUNT_CAR)
	if(D)
		pont = D.account_balance
	if (!isnum(pont))
		to_chat(user, "<span class='danger'>No active account on the console!</span>")
		return
	var/avail = S.available_shares
	var/price = S.current_value
	var/canbuy = round(pont / price)
	...
	amt = min(amt, S.available_shares, round(pont / S.current_value))
	if (!amt)
		return
