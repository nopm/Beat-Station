...
if [ "$BUILD_TOOLS" = false ]; then
	...
	if [ "$BUILD_TESTING" = true ]; then
		tools/travis/dm.sh -DTRAVISBUILDING -DTRAVISTESTING -DALL_MAPS beatstation.dme
	else
		tools/travis/dm.sh -DTRAVISBUILDING beatstation.dme
		...
		DreamDaemon beatstation.dmb -close -trusted -verbose -params "test-run&log-directory=travis"
		cd ..
