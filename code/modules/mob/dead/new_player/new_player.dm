...
/mob/dead/new_player
	...
	//var/pyko_hello // hippie -- pykoai // beat -- removes pykoai
...
/mob/dead/new_player/proc/AttemptLateSpawn(rank)
	...
	if(job && !job.override_latejoin_spawn(character))
		...
		if(!arrivals_docked)
			...
			/*var/picked_number = rand(1, 10) // hippie -- pykoai changes, also a random chance to make a joke about tgstation // beat start -- removes pykoai
			if(picked_number > 1)
				pyko_hello = 'hippiestation/sound/pyko/WelcomeHippie.ogg'
			else
				pyko_hello = 'hippiestation/sound/pyko/WelcomeUh.ogg'
			character.playsound_local(get_turf(character), pyko_hello, 70)*/ // beat end
