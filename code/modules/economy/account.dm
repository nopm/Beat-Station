...
/datum/bank_account/proc/bank_card_talk(message, force)
	...
	for(var/obj/A in bank_cards)
		var/icon_source = A // beat start
		if(istype(A, /obj/item/card/id))
			var/obj/item/card/id/id_card = A
			if(id_card.uses_overlays)
				icon_source = id_card.get_cached_flat_icon() // beat end
		...
		if(ismob(card_holder)) //If on a mob
			...
			if(card_holder.can_hear())
				to_chat(card_holder, "[icon2html(icon_source, card_holder)] *[message]*") // beat
			...
			for(var/mob/M in hearers(1,get_turf(A)))
				...
				A.audible_message("[icon2html(icon_source, hearers(A))] *[message]*", null, 1) // beat
				break
		else
			for(var/mob/M in A.loc) //If inside a container with other mobs (e.g. locker)
				...
				if(M.can_hear())
					to_chat(M, "[icon2html(icon_source, M)] *[message]*") // beat
