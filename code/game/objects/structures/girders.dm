/obj/structure/girder
	...
	var/next_beep = 0 //Prevents spamming of the construction sound // beat -- bepis
	max_integrity = 200
...
/obj/structure/girder/attackby(obj/item/W, mob/user, params)
	var/platingmodifier = 1 // beat start -- bepis
	if(HAS_TRAIT(user, TRAIT_QUICK_BUILD))
		platingmodifier = 0.7
		if(next_beep <= world.time)
			next_beep = world.time + 10
			playsound(src, 'sound/machines/clockcult/integration_cog_install.ogg', 50, TRUE) // beat end
	...
	if(istype(W, /obj/item/gun/energy/plasmacutter))
		...
		if(istype(S, /obj/item/stack/sheet/metal))
			if(state == GIRDER_DISPLACED)
				...
				if(do_after(user, 20*platingmodifier, target = src)) // beat -- bepis
					if(S.get_amount() < 2)
						return
					S.use(2)
					to_chat(user, "<span class='notice'>You create a false wall. Push on it to open or close the passage.</span>")
				...
				if(S.get_amount() < 2)
					...
					return
				to_chat(user, "<span class='notice'>You start adding plating...</span>")
				if (do_after(user, 40*platingmodifier, target = src)) // beat -- bepis
					if(S.get_amount() < 2)
						return
					S.use(2)
					to_chat(user, "<span class='notice'>You add the plating.</span>")
		...
		if(istype(S, /obj/item/stack/sheet/plasteel))
			if(state == GIRDER_DISPLACED)
				...
				if(state == GIRDER_REINF)
					...
					if(do_after(user, 50*platingmodifier, target = src)) // beat -- bepis
						if(S.get_amount() < 1)
							return
						S.use(1)
					...
					if(do_after(user, 60*platingmodifier, target = src)) // beat -- bepis
						if(S.get_amount() < 1)
							return
						S.use(1)
